...
from .forms import AttendanceForm, CustomPasswordChangeForm, RegistrationForm, LoginForm, SalaryRecordForm
...
def admin_dashboard(request):
    ...
    total_jars_out = JarInOut.objects.aggregate(total_out=Sum('jar_out'))['total_out'] or 0
    total_expenses = MonthlyExpense.objects.aggregate(total=Sum('amount'))['total'] or 0

    # Recent deliveries (latest 10)
    recent_deliveries = Delivery.objects.select_related('customer', 'driver').order_by('-date')[:10]
...
def staff_dashboard(request):
    ...
    jars_available = Jar.objects.aggregate(total_available=Sum('available_jars'))['total_available'] or 0
    jars_sold = Jar.objects.aggregate(total_sold=Sum('sold_jars'))['total_sold'] or 0
    jars_returned = Jar.objects.aggregate(total_returned=Sum('returned_jars'))['total_returned'] or 0
    jars_damaged = Jar.objects.aggregate(total_damaged=Sum('damaged_jars'))['total_damaged'] or 0
...
def driver_dashboard(request):
    ...
    # Get data for the driver dashboard
    total_jars = Jar.objects.aggregate(total=Sum('total_jars'))['total'] or 0
    returned_jars = Jar.objects.aggregate(total=Sum('returned_jars'))['total'] or 0
    damaged_jars = Jar.objects.aggregate(total=Sum('damaged_jars'))['total'] or 0

    # Get detailed records for the driver
    records = Delivery.objects.filter(driver__role='DRIVER').values('driver__full_name').annotate(
        jars_dispatched=Sum('quantity'),
    ...
    context = {
        ...
        'returned_jars': returned_jars,
        'damaged_jars': damaged_jars,
        'today_date': today_date,
        'records': records,
    }
    return render(request , 'account/driver_dashboard.html' , context)
...
def attendance(request):
    ...
    attendance_records = Attendance.objects.filter(check_in__range=(start_of_day, end_of_day))

    # Aggregate total hours worked
    total_hours_worked = attendance_records.aggregate(Sum('hours_worked'))['hours_worked__sum'] or 0
...
def add_attendance(request):
    ...
    existing_attendance = Attendance.objects.filter(user=request.user, check_in__date=today).first()

    if request.method == 'POST':
        if existing_attendance:
            form = AttendanceForm(request.POST, instance=existing_attendance, logged_in_user=request.user)
        else:
        ...
        if form.is_valid():
            ...
            if attendance.check_in and attendance.check_out:
                attendance.hours_worked = (attendance.check_out - attendance.check_in).total_seconds() / 3600
    ...
    return render(request, 'account/add_attendance.html', {
        'form': form,
        'is_edit': bool(existing_attendance),  # To indicate if editing an existing record
    })
...
def daily_record(request):
    ...
    driver_summaries = deliveries.values(
        'driver__id',
        'driver__full_name'
    ).annotate(
        total_jars=Sum('quantity'),
    ...
    for summary in driver_summaries:
        ...
        driver_list.append({
            ...
            'deliveries': driver_deliveries_dict[driver_id],  # attach the list of deliveries
        })
...
def manage_salaries(request):
    ...
    for worker in workers:
        ...
        total_hours_worked = Attendance.objects.filter(
            user=worker,
            check_in__year=year,
            check_in__month=month
        ).aggregate(total=Sum('hours_worked'))['total'] or 0
        ...
        salary_data.append({
            ...
            'base_salary': BASE_SALARY[worker.role] if total_hours_worked > 0 else 0,
...
def add_salary(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse('manage_salaries'))  # Redirect to the salary management page
    else:
