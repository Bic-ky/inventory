...
from .models import Bill, CreditMarket, Customer, Filler, JarCap, JarInOut, MonthlyExpense, Product, Vehicle, Driver, Delivery, Jar, Vendor , Filler
...
class DeliveryResource(resources.ModelResource):
    class Meta:
        model = Delivery
        fields = ('id', 'date', 'driver', 'customer', 'quantity', 'jar_type')
        

class JarResource(resources.ModelResource):
    class Meta:
        model = Jar
        fields = ('id', 'jar_type', 'total_jars', 'available_jars', 'damaged_jars', 'returned_jars')
...
class DeliveryAdmin(ImportExportModelAdmin):
    ...
    list_display = ('date', 'driver', 'customer', 'quantity')
