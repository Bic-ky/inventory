...
class Vehicle(models.Model):
    ...
    driver = models.ForeignKey('Driver', on_delete=models.CASCADE, related_name='vehicles', blank=True , null=True)  # Updated related_name
...
class Driver(models.Model):
    ...
    vehicle = models.ForeignKey('Vehicle', on_delete=models.CASCADE, related_name='drivers', blank=True , null=True)  # Updated related_name
...
class Delivery(models.Model):
    ...
    jar_type = models.CharField(max_length=1, choices=[('P', 'Premium'), ('N', 'Normal')], null=True, blank=True)
    sold_new_jars = models.IntegerField(default=0, help_text="Number of new jars sold during delivery")
    jar_price_at_delivery = models.DecimalField(max_digits=6, decimal_places=2, default=0.0, help_text="Price of each jar sold during this delivery")

    delivered_count = models.IntegerField(default=0, help_text="Number of jars successfully delivered")
    returned_count = models.IntegerField(default=0, help_text="Number of jars returned by the customer")
    leak_count = models.IntegerField(default=0, help_text="Number of jars found leaking")
    half_caps_count = models.IntegerField(default=0, help_text="Number of jars with half caps issues")
...
class Jar(models.Model):
    jar_type = models.CharField(max_length=1, choices=[('P', 'Premium'), ('N', 'Normal')] , default='Normal')
    ...
    jar_price = models.DecimalField(max_digits=6, decimal_places=2, default=0.0, help_text="Price per jar if sold")
...
class JarCap(models.Model):
    ...
    price_per_bora = models.DecimalField(
        max_digits=8,
        decimal_places=2,
        help_text="Price per bora of jar caps."
    )
    date_brought = models.DateField(
        default=datetime.now,
        help_text="Date when these jar caps were brought in."
    )

    last_updated = models.DateField(
        help_text="Date when we last changed this jar cap stock.",
        null=True,
        blank=True
    )
...
class MonthlyExpense(models.Model):
    ...
    remarks = models.TextField(blank=True, null=True, help_text="Additional remarks about the expense")
    ...
    miscellaneous_details = models.TextField(
        blank=True,
        null=True,
        help_text="Details for miscellaneous expenses"
    )
    ...
    def total_monthly_expense(cls, month, year):
        ...
        return cls.objects.filter(date__month=month, date__year=year).aggregate(total=models.Sum('amount'))['total'] or 0
